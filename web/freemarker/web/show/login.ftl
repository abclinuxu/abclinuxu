...
<table border=0 cellpadding=5>
  <tr>
    <td>Login: </td>
    <td>
     <input type="text" name="LOGIN" value="${PARAMS.LOGIN?if_exists}" size="8" tabindex="1" class="pole">
     <span class="error">${ERRORS.LOGIN?if_exists}</span>
    </td>
  </tr>
  <tr>
    <td>Heslo:</td>
    <td><input type="password" name="PASSWORD" size=8 tabindex=2 class="pole">
    <span class="error">${ERRORS.PASSWORD?if_exists}</span></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>
     <input type="submit" name="finish" value="OK" tabindex=3 class="buton">
    </td>
  </tr>
</table>
...
<form action="${URL.noPrefix("/SelectUser")}" method="POST">
 <input type="submit" value="Zapomnìl jsem heslo/login" class="buton">
