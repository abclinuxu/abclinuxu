...
<#if USER?? && TOOL.permissionsFor(USER, RELATION).canModify()>
    <p>
    <a href="${URL.noPrefix("/sprava/redakce/autori/edit/${AUTHOR.id}?action=edit")}">Upravit</a>

    <#if TOOL.permissionsFor(USER, RELATION).canDelete()>
        <a href="${URL.noPrefix("/sprava/redakce/autori/edit/${AUTHOR.id}?action=rm")}">Smazat</a>
    </#if>
