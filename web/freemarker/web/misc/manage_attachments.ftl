...
<form action="${URL.make("/inset/"+RELATION.id)}" method="POST" name="form">
    ...
    <#list ATTACHMENTS! as rel>
        ...
        <#if item.subtype??>mime typ: ${item.subtype}</#if>
        <#if node.@originalFilename[0]??>původní název souboru: ${node.@originalFilename[0]}</#if>
        <#if node.@size[0]??>velikost: ${node.@size[0]}</#if>
