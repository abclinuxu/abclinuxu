...
var IM_STOP = false; //umoňuje vypnutí reklamy
var IM_DEBUG = false;
var IM_CONTEXT_AD = true; //umoňuje vypnutí/zapnutí kontextové reklamy
...
function IM_init(page){
	...
	if(IM_adverLoaded){
		// Pokud server poslal nějakou hláku, vypíe ji
	...
	if (IM_DEBUG){
		window.setTimeout('window.status = "Debug on! - reklamní systém Impact"',2000); //zobrazí upozornění na debug mod
		if (typeof(page) != "number")	alert(IM_ERR_IDPAGE);
	}

	//
	// Načtení informací z prohlíeče uivatele.
	//
...
function IM_toGET(variable, variableName){
	if (typeof(variable) == "object"){			
		...
		for(var i in variable){
			if (i == 'copy') continue; //pravděpodobně BUG v JS, občas i nabýva tuto hodnotu,  ikdy není definováno 
...
//
// Funkce ověří, jestli byly volány pro vechny pozice funkce IM_pozice(). Ty které nebyly, vráti v poli.
//
...
function IM_urlEncode(str){
	str = escape(str);
	// nahradi znaky *@+/ příslunými entitami
	str = str.replace(/([*@+\/])/g, 
...
//
// Funkce se podívá do URL stránky a vyhledá vechny parametry, např. 'impact', které vrátí jako pole
//
