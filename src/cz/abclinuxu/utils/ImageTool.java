...
public class ImageTool {
	 ...
	 *            Image to to stored
	 * @param imageNo
	 *            Identification of image in object
	 * @param object
	 *            Object to which image belongs
	 * @param res
	 *            Image restrictions
	 * @param env
	 *            Storage for error messages
	 * @param errorKey
	 *            Key under which error messaged are filed
	 * @return {@code true} if stored successfully, {@code false} otherwise
	 */
	public static boolean storeImage(int imageNo, FileItem image,
			ImageAssignable object, ImageRestriction res, Map env,
			String errorKey) {
		if (image == null) {
			ServletUtils.addError(errorKey,
					res.getEMsg(ImageRestriction.EMPTY), env, null);
			return false;
		}

		String suffix = res.validateExtension(image.getName().toLowerCase());
		if (suffix == null) {
			ServletUtils.addError(errorKey, res
					.getEMsg(ImageRestriction.FILETYPE), env, null);
			return false;
		}
		...
		try {
			...
			if (!res.permitAnimation && reader.getNumImages(true) > 1) {
				ServletUtils.addError(errorKey, res
						.getEMsg(ImageRestriction.ANIMATED), env, null);
				return false;
			}
			if (reader.getHeight(0) > res.height
					|| reader.getWidth(0) > res.width) {
				ServletUtils.addError(errorKey, res
						.getEMsg(ImageRestriction.MAX_DIM), env, null);
				return false;
			}
		} catch (Exception e) {
			ServletUtils.addError(errorKey, res
					.getEMsg(ImageRestriction.IO_FAILED), env, null);
			return false;
		}
		...
		try {
			...
			ServletUtils.addError(errorKey, res
					.getEMsg(ImageRestriction.STORAGE_FAILED), env, null);
			log.error("Není možné uložit obrázek " + file.getAbsolutePath()
					+ " na disk!", e);
			return false;
		}
	...
	public static boolean deleteImage(int imageNo, ImageAssignable object) {
		...
		if (url != null) {
			...
			String localPath = AbcConfig
					.calculateDeployedPath(url.substring(1));
	 ...
	 */
	public static boolean createThumbnail(File imagePath, File thumbnailPath)
		...
		if (img == null)
			throw new AbcException("Nepodařilo se načíst obrázek ze souboru "
					+ imagePath);
	 ...
	 *            path to original image
	 * @param thumbnailPath
	 *            path for image to be created
	 * @param size
	 *            maximum width
	 * @param height
	 *            true if the size is height, false if the size is width
	 ...
	 *             failed to read image
	 */
	public static boolean createThumbnailMaxSize(File imagePath,
			File thumbnailPath, int size, boolean height) throws IOException,
			AbcException {
	 ...
	 */
	public static BufferedImage cutBottomIfNeeded(BufferedImage img, int width,
			int height) {
		int h;
	 ...
	 * @return new image, if it was created, or null
	 */
	public static BufferedImage scaleIfNeeded(BufferedImage img, int width,
			int height) {
	...
	public static void main(String[] args) throws Exception {
		...
		createThumbnail(imageFile, new File(imageFile.getParent(), fileName
				+ "_mini" + extension));
		createThumbnailMaxSize(imageFile, new File(imageFile.getParent(),
				fileName + "_mini_width" + extension), width, false);
		createThumbnailMaxSize(imageFile, new File(imageFile.getParent(),
				fileName + "_mini_height" + extension), height, true);
	}
	...
	public static final ImageRestriction USER_PHOTO_RES = new ImageRestriction() {
		{
			...
			this.errorMsgs = Arrays.asList("Vyberte soubor s Vaší fotografií!",
					"Soubor musí být typu JPG, PNG, GIF nebo JPEG!",
					"Fotografie přesahuje povolené maximální rozměry!",
					"Nelze načíst obrázek!", "Chyba při zápisu na disk!",
					"Animované obrázky nejsou povoleny!");
	...
	public static final ImageRestriction USER_AVATAR_RES = new ImageRestriction() {
		{
			...
			this.errorMsgs = Arrays.asList("Vyberte soubor s avatarem!",
					"Soubor musí být typu JPG, PNG, GIF nebo JPEG!",
					"Avatar přesahuje povolené maximální rozměry!",
					"Nelze načíst obrázek!", "Chyba při zápisu na disk!",
					"Animované obrázky nejsou povoleny!");
		}
	};
	...
	public static final ImageRestriction AUTHOR_PHOTO_RES = new ImageRestriction() {
		{
			...
			this.errorMsgs = Arrays.asList("Vyberte soubor s fotkou autora!",
					"Soubor musí být typu JPG, PNG, GIF nebo JPEG!",
					"Obrázek autora přesahuje povolené maximální rozměry!",
					"Nelze načíst obrázek!", "Chyba při zápisu na disk!",
					"Animované obrázky nejsou povoleny!");
		}
	};
