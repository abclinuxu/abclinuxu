...
public class FormFilter {
	...
	for (Map.Entry<String, FilterQualifier> entry : filters.entrySet()) {
		...
		sb.append("&amp;").append(entry.getKey()).append('=').append(
		    URLEncoder.encode(entry.getValue().formValue, "UTF-8"));
	    } catch (UnsupportedEncodingException uee) {
		log.error(
		    "Couldn't not convert form parameters to UTF-8 URL encoded string in string: " + entry.getValue().formValue,
		    uee);
		return null;
	    }
	}
	...
	if (AUTHORS_BY_NAME.equals(filterName)) {
	    filters.put(
		filterName,
		new FilterQualifier(new CompareCondition(Field.STRING1, Operation.LIKE, LIKE_PREFIX + param
		    + LIKE_SUFFIX), formValue));
	} else if (AUTHORS_BY_SURNAME.equals(filterName)) {
	    filters.put(
		filterName,
		new FilterQualifier(new CompareCondition(Field.STRING2, Operation.LIKE, LIKE_PREFIX + param
		    + LIKE_SUFFIX), formValue));
	} else if (AUTHORS_BY_ACTIVE.equals(filterName)) {
	    int value = Misc.parseInt(formValue, 0);
	    filters.put(
		filterName,
		new FilterQualifier(construct(
		    Field.NUMERIC2, value, Operation.EQUAL, 1, Operation.EQUAL, IsNullCheck.BEFORE_CAP), formValue));
		...
		filters.put(filterName, new FilterQualifier(construct(
		    Field.COUNTER, pair.single, Operation.SMALLER, 101, Operation.GREATER_OR_EQUAL,
		...
		filters.put(
		    filterName,
		    new FilterQualifier(new NestedCondition(list, LogicalOperation.AND), formValue));
	    } else {
	    ...
	    filters.put(
		filterName, new FilterQualifier(construct(
		    Field.WHEN, value, Operation.SMALLER_OR_EQUAL, cap, Operation.GREATER,
