...
public class FormFilter {
	 ...
	 * 
	 * @param params
	 *            Parameters passed with HTTP header
	 * @param filterNames
	 *            Which filters are applied
	 */
	public FormFilter(Map<? super Object, ? super Object> params,
			String... filterNames) {
	 ...
	 * 
	 * @param paramName
	 *            Name of parameter (or filter)
	 * @param qualifer
	 *            Qualifier if condition matches
	 * @param formValue
	 *            Value passed from form
	 * @return Modified instance of filter
	 */
	public FormFilter appendFilterQualifier(String paramName,
			Qualifier qualifer, String formValue) {
	...
	private void initFilter(Object param, String filterName) {
		...
		if (AUTHORS_BY_NAME.equals(filterName)) {
			filters.put(filterName, new FilterQualifier(new CompareCondition(
					Field.STRING1, Operation.LIKE, LIKE_PREFIX + param
							+ LIKE_SUFFIX), formValue));
		} else if (AUTHORS_BY_SURNAME.equals(filterName)) {
			filters.put(filterName, new FilterQualifier(new CompareCondition(
					Field.STRING2, Operation.LIKE, LIKE_PREFIX + param
							+ LIKE_SUFFIX), formValue));
		} else if (AUTHORS_BY_ACTIVE.equals(filterName)) {
			int value = Misc.parseInt(formValue, 0);
			filters.put(filterName, new FilterQualifier(construct(
					Field.NUMERIC2, value, Operation.EQUAL, 1, Operation.EQUAL,
					IsNullCheck.BEFORE_CAP), formValue));
		} else if (AUTHORS_BY_ARTICLES.equals(filterName)) {
			int value = Misc.parsePossiblyWrongInt(formValue);
			filters.put(filterName, new FilterQualifier(construct(
					Field.COUNTER, value, Operation.SMALLER, 101,
					Operation.GREATER_OR_EQUAL, IsNullCheck.BEFORE_CAP),
					formValue));
			...
			filters.put(filterName, new FilterQualifier(construct(Field.WHEN,
					value, Operation.SMALLER_OR_EQUAL, cap, Operation.GREATER,
					IsNullCheck.NO_CHECK), formValue));
	 ...
	 * 
	 * @param <T>
	 *            Type of cap and value
	 * @param field
	 *            Field to be compared
	 * @param value
	 *            Value passed from form
	 * @param before
	 *            Operation applied to comparison before cap (excluded) is
	 *            reached
	 * @param cap
	 *            Value of cap
	 * @param after
	 *            Operation applied to comparison after cap (included) is
	 *            reached
	 * @param inc
	 *            How to determine IS NULL presence
	 * @return Qualifier for given field
	 */
	private <T extends Comparable<? super T>> Qualifier construct(Field field,
			T value, Operation before, T cap, Operation after, IsNullCheck inc) {
