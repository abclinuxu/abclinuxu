...
public class User extends CommonObject implements ImageAssignable {
	...
	public boolean isMemberOf(int group) {
		return getData().selectSingleNode(
				"/data/system/group[text()='" + group + "']") != null;
	}
	...
	public boolean isMemberOf(String groupName) {
		List<Item> items = SQLTool.getInstance().findItemsWithType(Item.GROUP,
				0, EditGroup.DEFAULT_MAX_NUMBER_OF_GROUPS);
	...
	/**
	 * Fills last seen comments for this user. This is cache to be filled from
	 * persistent storage, changes are not propagated to persistence.
	 * 
	 * @param map
	 *            map to be copied, key is discussion id (integer), value is
	 *            last seen comment id (integer).
	 */
	...
	private void createLastSeenCommentsMap() {
		lastSeenDiscussions = new LRUMap(AbcConfig
				.getMaxWatchedDiscussionLimit());
	}
	 ...
	 * @param discussion
	 *            id of discussion
	 * @return id of last seen comment or null, if he has not opened that
	 *         dicussion yet
	 */
	...
	public void assignImage(int imageNo, String imageUrl) {
		...
		switch (imageNo) {
		case USER_AVATAR:
			image = DocumentHelper.makeElement(getData(),
					"/data/profile/avatar");
			break;
		case USER_PHOTO:
			image = DocumentHelper
					.makeElement(getData(), "/data/profile/photo");
			break;
		default:
			return;
		}
	...
	public String proposeImageUrl(int imageNo, String suffix) {
		...
		switch (imageNo) {
		case USER_AVATAR:
			return sb.append("images/avatars/").append(id).append('.').append(
					suffix).toString();
		case USER_PHOTO:
			return sb.append("images/faces/").append(id).append('.').append(
					suffix).toString();
		default:
			return null;
		}
	...
	/**
	 * Compares content fields of this and that GenericObject. The argument must
	 * be instance of same class and have same content properties.
	 * 
	 * @param obj
	 *            compared class
