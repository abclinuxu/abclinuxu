...
public class EditGuestBook implements AbcAction {
    ...
    boolean setMessage(Map params, Element root, Map env) {
        ...
        if (Misc.empty(content)) {
            ServletUtils.addError(PARAM_MESSAGE, "Prosím zadejte váš vzkaz.", env, null);
            return false;
        }
    ...
    boolean setAuthor(Map params, Element root, Map env) {
        ...
        if (Misc.empty(name)) {
            ServletUtils.addError(PARAM_NAME, "Prosím zadejte vaše jméno.", env, null);
            return false;
        }
        if (name.indexOf('<')!=-1) {
            ServletUtils.addError(PARAM_NAME, "Jméno nesmí obsahovat HTML značky.", env, null);
            return false;
        }
