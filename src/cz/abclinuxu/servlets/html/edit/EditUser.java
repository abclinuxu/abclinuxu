...
public class EditUser implements AbcAction {
	...
	public String process(HttpServletRequest request,
		...
		User managed = (User) InstanceUtils.instantiateParam(PARAM_USER_SHORT,
				User.class, params, request);
		User user = (User) env.get(Constants.VAR_USER);
		if (managed == null)
			managed = user;
		else
			managed = (User) PersistenceFactory.getPersistence().findById(
					managed);
		if (managed != null)
		...
		if (action == null || action.equals(ACTION_REGISTER))
			return FMTemplateSelector.select("EditUser", "register", env,
					request);
		else if (action.equals(ACTION_REGISTER_STEP2)) {
			ActionProtector.ensureContract(request, EditUser.class, false,
					true, true, false);
			return actionAddStep2(request, response, env);
		}
		if (ACTION_FORGOTTEN_PASSWORD.equals(action))
			return FMTemplateSelector.select("EditUser", "forgottenPassword",
					env, request);
		...
		if (action.equals(ACTION_CHANGE_FORGOTTEN_PASSWORD))
			return FMTemplateSelector.select("EditUser",
					"changeForgottenPassword", env, request);
		...
		// all other actions require user to be logged in and to have rights for
		// this action
		if (user == null) {
		...
		if (action.equals(ACTION_ADD_GROUP_MEMBER)) {
			ActionProtector.ensureContract(request, EditUser.class, true,
					false, false, true);
		...
		if (!(user.getId() == managed.getId() || user.hasRole(Roles.USER_ADMIN)))
			return FMTemplateSelector.select("ViewUser", "forbidden", env,
					request);
		...
		if (action.equals(ACTION_EDIT_BASIC_STEP2)) {
			ActionProtector.ensureContract(request, EditUser.class, true, true,
					true, false);
			return actionEditBasic2(request, response, env);
		}

		if (action.equals(ACTION_CHANGE_PASSWORD))
			return FMTemplateSelector.select("EditUser", "changePassword", env,
					request);
		...
		if (action.equals(ACTION_UPLOAD_PHOTO))
			return FMTemplateSelector.select("EditUser", "uploadPhoto", env,
					request);
		...
		if (action.equals(ACTION_UPLOAD_AVATAR))
			return FMTemplateSelector.select("EditUser", "uploadAvatar", env,
					request);
		...
		if (action.equals(ACTION_EDIT_GPG))
			return FMTemplateSelector.select("EditUser", "editGPG", env,
					request);
		...
		if (user.hasRole(Roles.USER_ADMIN)
				|| user.hasRole(Roles.CAN_INVALIDATE_EMAILS)) {
			if (action.equals(ACTION_INVALIDATE_EMAIL))
				return FMTemplateSelector.select("EditUser", "invalidateEmail",
						env, request);
		...
		if (action.equals(ACTION_REMOVE_MERGE)) {
			return FMTemplateSelector.select("EditUser", "removeMerge", env,
					request);
		}
	 ...
	 * Creates new user.
	 */
	protected String actionAddStep2(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		...
		managed.addProperty(Constants.PROPERTY_TICKET, generateTicket(managed
				.getId()));
		...
		try {
			ldapManager.registerUser(managed.getLogin(), managed.getPassword(),
					null, managed.getName(), LdapUserManager.SERVER_ABCLINUXU);
			persistence.create(managed);
		} catch (DuplicateKeyException e) {
			ServletUtils.addError(PARAM_LOGIN,
					"Přihlašovací jméno nebo přezdívka jsou již používány.",
					env, null);
			return FMTemplateSelector.select("EditUser", "register", env,
					request);
		}
		...
		if (managed.getEmail() != null) {
			...
			changes
					.put(LdapUserManager.ATTRIB_EMAIL_ADRESS, managed
							.getEmail());
			...
			data.put(EmailSender.KEY_RECEPIENT_UID, Integer.toString(managed
					.getId()));
			data.put(EmailSender.KEY_SUBJECT,
					"Privitani na portalu www.abclinuxu.cz");
			data.put(EmailSender.KEY_TEMPLATE, "/mail/registrace.ftl");
			EmailSender.sendEmail(data);
		}

		UrlUtils urlUtils = (UrlUtils) env.get(Constants.VAR_URL_UTILS);
		urlUtils.redirect(response, "/Profile?registrace=true&action="
				+ ViewUser.ACTION_SHOW_MY_PROFILE + "&uid=" + managed.getId());
		return null;
	}
	 ...
	 */
	protected String actionEditBasic(HttpServletRequest request, Map env)
			throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
	 ...
	 */
	protected String actionEditBasic2(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
		User user = (User) env.get(Constants.VAR_USER);
		...
		if (!canContinue)
			return FMTemplateSelector.select("EditUser", "editBasic", env,
					request);
		...
		try {
			...
			if (managed.getEmail() != null) {
				changes.put(LdapUserManager.ATTRIB_EMAIL_ADRESS, managed
						.getEmail());
				Node node = managed.getData().selectSingleNode(
						"/data/communication/email/@valid");
				String value = (node != null && "no".equalsIgnoreCase(node
						.getText())) ? "true" : "false";
				changes.put(LdapUserManager.ATTRIB_EMAIL_BLOCKED, value);
				node = managed.getData().selectSingleNode(
						"/data/communication/email/@verified");
				value = (node == null || "yes".equalsIgnoreCase(node.getText())) ? "true"
						: "false";
			...
			ServletUtils.addError(PARAM_LOGIN,
					"Login nebo přezdívka je již používána", env, null);
			return FMTemplateSelector.select("EditUser", "editBasic", env,
					request);
		}
		...
		ServletUtils.addMessage("Změny byly uloženy.", env, request
				.getSession());
		UrlUtils urlUtils = (UrlUtils) env.get(Constants.VAR_URL_UTILS);
		urlUtils.redirect(response, "/Profile?action="
				+ ViewUser.ACTION_SHOW_MY_PROFILE + "&uid=" + managed.getId());
		return null;
	}
	 ...
	 */
	protected String actionPassword2(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
		User user = (User) env.get(Constants.VAR_USER);
		...
		ServletUtils.addMessage("Heslo bylo změněno.", env, request
				.getSession());
		UrlUtils urlUtils = (UrlUtils) env.get(Constants.VAR_URL_UTILS);
		urlUtils.redirect(response, "/Profile?action="
				+ ViewUser.ACTION_SHOW_MY_PROFILE + "&uid=" + managed.getId());
	...
	protected String actionChangeForgottenPassword2(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
		...
		if (token == null || token.length() == 0) {
			ServletUtils.addError(Constants.ERROR_GENERIC, "Chybí token!", env,
					request.getSession());
			return FMTemplateSelector.select("EditUser",
					"changeForgottenPassword", env, request);
		}
		...
		ldapManager.resetPassword(managed.getLogin(), token, managed
				.getPassword());

		ServletUtils.addMessage(
				"Heslo bylo změněno, nyní se můžete přihlásit.", env, request
						.getSession());
	 ...
	 * Shows form for editing of basic information about account.
	 */
	protected String actionEditPersonal(HttpServletRequest request, Map env)
			throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
		...
		return FMTemplateSelector.select("EditUser", "editPersonal", env,
				request);
	}
	 ...
	 * Updates basic information about account.
	 */
	protected String actionEditPersonal2(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
		User user = (User) env.get(Constants.VAR_USER);
	 ...
	 * Shows form for editing of basic information about account.
	 */
	protected String actionEditProfile(HttpServletRequest request, Map env)
			throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
		...
		List nodes = Tools.xpaths(document,
				"/data/profile/distributions/distribution");
		if (nodes.size() > 0)
			params.put(PARAM_DISTRIBUTION, nodes);

		return FMTemplateSelector.select("EditUser", "editProfile", env,
				request);
	}
	 ...
	 * Updates basic information about account.
	 */
	protected String actionEditProfile2(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
		User user = (User) env.get(Constants.VAR_USER);
	 ...
	 */
	protected String actionEditSettings(HttpServletRequest request, Map env)
			throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
		...
		params.put(PARAM_NEWS_MULTILINE, (node != null) ? node.getText()
				: "yes");
		...
		Element element = (Element) document
				.selectSingleNode("/data/settings/feeds");
		if (element != null) {
		...
		return FMTemplateSelector.select("EditUser", "editSettings", env,
				request);
	}
	 ...
	 */
	protected String actionEditSettings2(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
		User user = (User) env.get(Constants.VAR_USER);
		...
		UrlUtils urlUtils = (UrlUtils) env.get(Constants.VAR_URL_UTILS);
		urlUtils.redirect(response, "/Profile?action="
				+ ViewUser.ACTION_SHOW_MY_PROFILE + "&userId="
				+ managed.getId());
		return null;
	}
	...
	private void setDefaultValuesForEditSettings(Map env) {
		...
		env.put(VAR_DEFAULT_DISCUSSION_COUNT, defaultSizes
				.get(VariableFetcher.KEY_QUESTION));
		env.put(VAR_DEFAULT_SCREENSHOT_COUNT, defaultSizes
				.get(VariableFetcher.KEY_SCREENSHOT));
		env.put(VAR_DEFAULT_NEWS_COUNT, defaultSizes
				.get(VariableFetcher.KEY_NEWS));
		env.put(VAR_DEFAULT_STORIES_COUNT, defaultSizes
				.get(VariableFetcher.KEY_STORY));
		env.put(VAR_DEFAULT_FEED_LINKS_COUNT, defaultSizes
				.get(VariableFetcher.KEY_TEMPLATE_LINKS));
		env.put(VAR_DEFAULT_TEMPLATE_FEED_LINKS_COUNT, defaultSizes
				.get(VariableFetcher.KEY_INDEX_LINKS));
	}

	protected String actionEditBlacklist(HttpServletRequest request, Map env)
			throws Exception {
		return FMTemplateSelector.select("EditUser", "editBlacklist", env,
				request);
	}
	 ...
	 */
	protected String actionAddToBlacklist(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		UrlUtils urlUtils = (UrlUtils) env.get(Constants.VAR_URL_UTILS);
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
		...
		if (url == null || url.length() == 0)
			url = "/EditUser/" + managed.getId() + "?action="
					+ ACTION_EDIT_BLACKLIST;
		...
		ServletUtils.addMessage(
				"Uživatel byl přidán na seznam blokovaných uživatelů.", env,
				request.getSession());
		urlUtils.redirect(response, url);
		return null;
	}
	 ...
	 */
	protected String actionRemoveFromBlacklist(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		UrlUtils urlUtils = (UrlUtils) env.get(Constants.VAR_URL_UTILS);
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
		...
		ServletUtils.addMessage(
				"Uživatel byl odstraněn ze seznamu blokovaných uživatelů.",
	 ...
	 */
	protected String actionEditSubscription(HttpServletRequest request, Map env)
			throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
		...
		Document document = managed.getData();
		Node node = document
				.selectSingleNode("/data/communication/email/weekly_summary");
		if (node != null)
			params.put(PARAM_SUBSCRIBE_WEEKLY, node.getText());
		node = document
				.selectSingleNode("/data/communication/email/newsletter");
		if (node != null)
		...
		return FMTemplateSelector.select("EditUser", "editSubscription", env,
				request);
	}
	 ...
	 */
	protected String actionEditSubscription2(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
		User user = (User) env.get(Constants.VAR_USER);
		...
		if (!canContinue)
			return FMTemplateSelector.select("EditUser", "editSubscription",
					env, request);
	 ...
	 * Uploads photo.
	 */
	protected String actionUploadPhoto2(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
		User user = (User) env.get(Constants.VAR_USER);
	...
	protected String actionChangeForumMode(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		User managed = (User) env.get(VAR_MANAGED);
		Map params = (Map) env.get(Constants.VAR_PARAMS);
	...
	protected String actionChangeStyle(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		URL referer = ServletUtils.getReferer(request);
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
	...
	protected String actionEditGPG2(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
	 ...
	 * Uploads photo.
	 */
	protected String actionUploadAvatar2(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
		User user = (User) env.get(Constants.VAR_USER);
	 ...
	 */
	protected String actionRemoveMerge2(HttpServletRequest request, Map env)
			throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		...
		if (uid1 == uid2) {
			ServletUtils.addError(Constants.ERROR_GENERIC, "UID jsou shodná!",
					env, null);
			return FMTemplateSelector.select("EditUser", "removeMerge", env,
					request);
		}
		...
		try {
			...
			if (user1 == null) {
				ServletUtils.addError(PARAM_UID1, "První UID je neplatné!",
						env, null);
				return FMTemplateSelector.select("EditUser", "removeMerge",
						env, request);
			} else {
				ServletUtils.addError(PARAM_UID2, "Druhé UID je neplatné!",
						env, null);
				return FMTemplateSelector.select("EditUser", "removeMerge",
						env, request);
			}
		...
		if (user2 == null) {
			...
			Qualifier[] qualifiers = new Qualifier[] { new CompareCondition(
					Field.NUMERIC1, Operation.EQUAL, user1.getId()) };
			List<Relation> authors = sqlTool.findItemRelationsWithType(
					Item.AUTHOR, qualifiers);
			...
			if (!unused) {
				ServletUtils
						.addError(
								PARAM_UID1,
								"Tento uživatel má záznamy v systému! Musíte je nejdříve smazat.",
								env, null);
				return FMTemplateSelector.select("EditUser", "removeMerge",
						env, request);
			}
		...
		return FMTemplateSelector.select("EditUser", "removeMerge2", env,
				request);
	}
	 ...
	 */
	protected String actionRemoveMerge3(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		User user = (User) env.get(Constants.VAR_USER);
		SQLTool sqlTool = SQLTool.getInstance();
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		...
		if (uid2 != 0) {
			ServletUtils.addMessage("Uživatelé byli sloučeni.", env, request
					.getSession());
			AdminLogger.logEvent(user, "sloucil uzivatele " + uid1 + "("
					+ user1.getEmail() + ") s uzivatelem " + uid2);
		} else {
			ServletUtils.addMessage("Uživatel byl odstraněn.", env, request
					.getSession());
			;
			AdminLogger.logEvent(user, "smazal uzivatele " + uid1 + "("
					+ user1.getEmail() + ")");
		}
	 ...
	 */
	protected String actionGrant(HttpServletRequest request, Map env)
			throws Exception {
		env.put(VAR_USERS, SQLTool.getInstance().findUsersWithRoles(null));
		return FMTemplateSelector.select("EditUser", "grantSelect", env,
				request);
	}
	 ...
	 * Shows form for granting roles to the user.
	 */
	protected String actionGrant2(HttpServletRequest request, Map env)
			throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
		...
		return FMTemplateSelector
				.select("EditUser", "grantRoles", env, request);
	}

	/**
	 * Manages roles of the user.
	 */
	protected String actionGrant3(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
		User user = (User) env.get(Constants.VAR_USER);
		...
		if (!canContinue)
			return FMTemplateSelector.select("EditUser", "grantRoles", env,
					request);
		...
		ServletUtils.addMessage("Změny rolí uživatele " + managed.getName()
				+ " byly uloženy.", env, request.getSession());
		UrlUtils urlUtils = (UrlUtils) env.get(Constants.VAR_URL_UTILS);
		urlUtils.redirect(response, "/Profile?action="
				+ ViewUser.ACTION_SHOW_MY_PROFILE + "&userId="
				+ managed.getId());
	 ...
	 */
	protected String actionInvalidateEmail(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User user = (User) env.get(Constants.VAR_USER);
		Persistence persistence = PersistenceFactory.getPersistence();
		...
		while (stk.hasMoreTokens()) {
			...
			try {
				...
				Element tagEmail = DocumentHelper.makeElement(
						managed.getData(), "/data/communication/email");
				tagEmail.attribute("valid").setText("no");
				persistence.update(managed);
				AdminLogger.logEvent(user, "zneplatnil email uzivateli "
						+ managed.getName() + " - " + managed.getId());
				count++;
			} catch (Exception e) {
		...
		if (sb.length() > 0)
			ServletUtils.addError(Constants.ERROR_GENERIC, sb.toString(), env,
					request.getSession());
		ServletUtils.addMessage(count + " uživatelům byl zneplatněn email.",
				env, request.getSession());
		UrlUtils urlUtils = (UrlUtils) env.get(Constants.VAR_URL_UTILS);
		urlUtils.redirect(response, "/Admin");
		return null;
	 ...
	 */
	protected String actionAddToGroup(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		User managed = (User) env.get(VAR_MANAGED);
		Persistence persistence = PersistenceFactory.getPersistence();
		...
		if (gid == 0)
			return ServletUtils.showErrorPage("Chybí číslo skupiny!", env,
					request);
		...
		Element system = (Element) managed.getData().selectSingleNode(
				"/data/system");

		if (system == null) {
			Element data = (Element) managed.getData()
		...
		AdminLogger.logEvent(user, "vlozil uzivatele " + managed.getId()
				+ " do skupiny " + gid);
	...
	protected String actionToggleForumHP(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		...
		Map<Integer, Integer> defaultForums = VariableFetcher.getInstance()
				.getMainForums();

		boolean isSystem = defaultForums.containsKey(rid);

		Element forum = (Element) user.getData().selectSingleNode(
				"/data/forums/forum[text()='" + rid + "']");
		...
		if (forums.containsKey(rid) && forums.get(rid) > 0) {
			...
			if (forum != null) {
				...
				Element forumsElem = DocumentHelper.makeElement(user.getData(),
						"/data/forums");
			...
			if (isSystem)
				...
				questions = VariableFetcher.getInstance().getDefaultSizes()
						.get(VariableFetcher.KEY_QUESTION);
	...
	protected String actionForgottenPasswordStep2(HttpServletRequest request,
			HttpServletResponse response, Map env) throws Exception {
		Map params = (Map) env.get(Constants.VAR_PARAMS);
		String login = (String) params.get(PARAM_LOGIN);

		if (login == null || login.length() == 0) {
			ServletUtils.addError(PARAM_LOGIN, "Zadejte uživatelské jméno!",
					env, null);
			return FMTemplateSelector.select("EditUser", "forgottenPassword",
					env, request);
		}
		...
		if (id == null || id == 0) {
			ServletUtils
					.addError(PARAM_LOGIN, "Uživatel nenalezen!", env, null);
			return FMTemplateSelector.select("EditUser", "forgottenPassword",
					env, request);
		}
		...
		if (managed.getEmail() == null) {
			ServletUtils.addError(PARAM_LOGIN,
					"Uživatel nemá přiřazenou e-mailovou adresu!", env, null);
			return FMTemplateSelector.select("EditUser", "forgottenPassword",
					env, request);
		}

		String token = generateTicket(managed.getId());
		Map<String, String> changes = Collections.singletonMap(
				LdapUserManager.ATTRIB_FORGOTTEN_PASSWORD_TOKEN, token);
		ldapManager.updateUser(managed.getLogin(), changes);
		...
		ServletUtils.addMessage("E-mail byl odeslán.", env, request
				.getSession());
	 ...
	 *            text to be checked
	 * @param paramName
	 *            error message will be associated with this parameter
	 * @return true if everything is ok, false if guard throws exception
	 */
	private static boolean verifyGuard(Rules rules, String text,
			String paramName, Map env) {
		try {
	...
	/**
	 * Performs anti-spambot detection. Logged in user or already verified user
	 * is automatically allowed to submit comment. Other users must enter
	 * current year. In such case new cookie is created, which will hold their
	 * name and its existence will show that anti-spambot detection was already
	 * successfully performed for this user.
	 * 
	...
	public boolean checkSpambot(Map params, Map env, User user) {
		...
		if (!String.valueOf(year).equals(s)) {
			ServletUtils.addError(PARAM_ANTISPAM,
					"Zadejte prosím letošní rok.", env, null);
			return false;
		}
		return true;
	}
	 ...
	 * 
	 * @param params
	 *            map holding request's parameters
	 * @param user
	 *            user to be updated
	 * @param env
	 *            environment
	...
	private boolean checkPassword(Map params, User user, Map env) {
		...
		if (password == null || password.length() == 0) {
			ServletUtils.addError(ServletUtils.PARAM_LOG_PASSWORD,
					"Zadejte heslo!", env, null);
			return false;
		}
		if (!ldapManager.login(user.getLogin(), password,
				LdapUserManager.SERVER_ABCLINUXU)) {
			ServletUtils.addError(ServletUtils.PARAM_LOG_PASSWORD,
					"Nesprávné heslo!", env, null);
			return false;
		}
		return true;
	}
	...
	/**
	 * Updates password from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean setPassword(Map params, User user, Map env) {
		...
		if (password == null || password.length() < 4) {
			ServletUtils.addError(PARAM_PASSWORD, "Heslo je příliš krátké!",
					env, null);
			return false;
		}
	...
	/**
	 * Updates login from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean setLogin(Map params, User user, Map env) {
		...
		if (login == null || login.length() < 3) {
			ServletUtils
					.addError(PARAM_LOGIN,
							"Přihlašovací jméno musí mít nejméně tři znaky!",
							env, null);
			return false;
		}
		...
		if (matcher.find()) {
			ServletUtils
					.addError(
							PARAM_LOGIN,
							"Přihlašovací jméno smí obsahovat pouze písmena A až Z, číslice, pomlčku, tečku a podtržítko!",
							env, null);
			return false;
		}

		List<Integer> users = SQLTool.getInstance().findUsersWithLogin(login,
				null);
		if (!(users.isEmpty() || users.contains(user.getId()))) {
			ServletUtils.addError(PARAM_LOGIN,
					"Toto přihlašovací jméno je již používáno. Zkuste jiné.",
					env, null);
			return false;
		}
	...
	/**
	 * Updates name from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	public static boolean setName(Map params, User user, Map env) {
		...
		if (name == null || name.length() < 3) {
			ServletUtils.addError(PARAM_NAME, "Jméno je příliš krátké!", env,
					null);
			return false;
		}
	...
	/**
	 * Updates openid from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	/**
	 * Updates nick from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean setNick(Map params, User user, Map env) {
		...
		nick = nick.trim();
		if (nick.length() > 20) {
			ServletUtils.addError(PARAM_NICK, "Přezdívka je příliš dlouhá!",
					env, null);
			return false;
		}
		...
		List<Integer> users = SQLTool.getInstance().findUsersWithNick(nick,
				null);
		if (!(users.isEmpty() || users.contains(user.getId()))) {
			ServletUtils
					.addError(PARAM_NICK,
							"Tato přezdívka je již používána. Zkuste jinou.",
							env, null);
			return false;
		}
	...
	/**
	 * Updates email from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	public static boolean setEmail(Map params, User user, Map env) {
		...
		if (email == null || email.trim().length() == 0) {
			user.setEmail(null);
			Element tagEmail = (Element) user.getData().selectSingleNode(
		...
		Element tagEmail = DocumentHelper.makeElement(user.getData(),
				"/data/communication/email");
	...
	/**
	 * Updates sex from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	public static boolean setSex(Map params, User user) {
		...
		Node node = DocumentHelper.makeElement(user.getData(),
				"/data/personal/sex");
		node.setText(sex);
		return true;
	}
	...
	/**
	 * Updates birth year from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean setBirthYear(Map params, User user, Map env) {
		...
		Element personal = DocumentHelper.makeElement(user.getData(),
				"/data/personal");
		...
		if (valid == -1) {
			ServletUtils.addError(PARAM_BIRTH_YEAR, "Zadejte platný rok!", env,
					null);
			return false;
		}
	...
	/**
	 * Updates city from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	/**
	 * Updates area from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	/**
	 * Updates country from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	/**
	 * Updates home page from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	public static boolean setMyPage(Map params, User user, Map env) {
		...
		Element profile = DocumentHelper.makeElement(user.getData(),
				"/data/profile");
	...
	/**
	 * Updates linuxUserFromYear from parameters. Changes are not synchronized
	 * with persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean setLinuxUserFrom(Map params, User user, Map env) {
		...
		if (year < 1991) {
			ServletUtils.addError(PARAM_LINUX_USER_FROM,
					"Očekáváno je číslo větší nebo rovno 1991.", env, null);
			return false;
		}
	...
	/**
	 * Updates about from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	/**
	 * Updates distributions from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean setDistributions(Map params, User user, Map env) {
		...
		if (distros == null || distros.size() == 0)
			return true;
		Element distributions = DocumentHelper.makeElement(profile,
				"distributions");
	...
	/**
	 * Updates signature from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean setSignature(Map params, User user, Map env)
		...
		if (contentSize(signature) > 120) {
			ServletUtils.addError(PARAM_SIGNATURE,
					"Maximální délka je 120 znaků!", env, null);
			return false;
		}
	...
	/**
	 * Updates URL with custom CSS from parameters. Changes are not synchronized
	 * with persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean setCssUrl(Map params, User user) {
		...
		if (url == null || url.length() == 0) {
			...
			Element element = DocumentHelper.makeElement(user.getData(),
					"/data/settings/css");
			element.setText(url);
		}
		return true;
	...
	/**
	 * Updates emoticons from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	/**
	 * Updates rich text editor settings from parameters. Changes are not
	...
	/**
	 * Updates signatures from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	/**
	 * Updates avatars from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	/**
	 * Updates news title display settings from parameters. Changes are not
	 * synchronized with persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	/**
	 * Updates banned blogs view settings from parameters. Changes are not
	 * synchronized with persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	/**
	 * /** Updates guidepost from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	/**
	 * Updates login cookie validity from parameters. Changes are not
	 * synchronized with persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean setCookieValidity(Map params, User user) {
		...
		Element element = DocumentHelper.makeElement(user.getData(),
				"/data/settings/cookie_valid");
		element.setText(validity);
		return true;
	}

	/**
	 * Updates size limit of discussions on main page from parameters. Changes
	 * are not synchronized with persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	 * @param user
	...
	private boolean setDiscussionsSizeLimit(Map params, User user, Map env) {
		...
		Element modeElem = (Element) user.getData().selectSingleNode(
				"/data/profile/forum_mode");
		...
		if (!singleMode) {
			Element forumsElem = DocumentHelper.makeElement(user.getData(),
					"/data/forums");
			Map<Integer, Integer> mainForums = VariableFetcher.getInstance()
					.getMainForums();
			Map<Integer, Integer> forums = Tools.getUserForums(user);
			...
			for (Integer rid : forums.keySet()) {
				...
				if (value < 0 || value > max) {
					ServletUtils.addError(paramName,
							"Zadejte číslo v rozsahu 0 - " + max + "!", env,
							null);
					ok = false;
					continue;
				}

				Element elem = (Element) forumsElem
						.selectSingleNode("forum[text()='" + rid + "']");
				if (elem == null) {
			...
			return setLimitedSize(params, PARAM_DISCUSSIONS_COUNT, user
					.getData(), "/data/settings/index_discussions", 0, max, env);
		}
	...
	/**
	 * Updates size limit of screenshots on main page from parameters. Changes
	 * are not synchronized with persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean setScreenshotsSizeLimit(Map params, User user, Map env) {
		...
		return setLimitedSize(params, PARAM_SCREENSHOTS_COUNT, user.getData(),
				"/data/settings/index_screenshots", 0, max, env);
	}

	/**
	 * Updates size limit for news in the template from parameters. Changes are
	 * not synchronized with persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	 * @param user
	...
	private boolean setNewsSizeLimit(Map params, User user, Map env) {
		...
		return setLimitedSize(params, PARAM_NEWS_COUNT, user.getData(),
				"/data/settings/index_news", 0, max, env);
	}

	/**
	 * Updates size limit for stories on index page from parameters. Changes are
	 * not synchronized with persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean setStoriesSizeLimit(Map params, User user, Map env) {
		...
		return setLimitedSize(params, PARAM_STORIES_COUNT, user.getData(),
				"/data/settings/index_stories", 0, max, env);
	}

	/**
	 * Updates default page size from parameters. Changes are not synchronized
	 * with persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean setDefaultPageSize(Map params, User user, Map env) {
		return setLimitedSize(params, PARAM_DEFAULT_PAGE_SIZE, user.getData(),
				"/data/settings/page_size", 10, 100, env);
	}

	/**
	 * Updates page size for found objects from parameters. Changes are not
	 * synchronized with persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean setFoundPageSize(Map params, User user, Map env) {
		return setLimitedSize(params, PARAM_FOUND_PAGE_SIZE, user.getData(),
				"/data/settings/found_size", 10, 100, env);
	}

	/**
	 * Updates page size for forum from parameters. Changes are not synchronized
	 * with persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean setForumPageSize(Map params, User user, Map env) {
		return setLimitedSize(params, PARAM_FORUM_PAGE_SIZE, user.getData(),
				"/data/settings/forum_size", 10, 100, env);
	}

	/**
	 * Adds a uid to users blacklist. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean addToBlacklist(Map params, User user, Map env) {
		...
		Element blacklist = DocumentHelper.makeElement(user.getData(),
		...
		if (!paramFound) {
			ServletUtils.addError(PARAM_BLACKLIST_USER, "Chybí parametr "
					+ PARAM_BLACKLIST_USER + "!", env, null);
			return false;
		}
		return true;
	}
	...
	/**
	 * Removes a uid from users blacklist. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean removeFromBlacklist(Map params, User user, Map env) {
		...
		Element blacklist = (Element) user.getData().selectSingleNode(
				"/data/settings/blacklist");
		...
		for (Iterator iter = names.iterator(); iter.hasNext();) {
			...
			String s = (String) iter.next();
			Node node = blacklist
					.selectSingleNode("name[text()=\"" + s + "\"]");
			if (node != null)
				node.detach();
		}

		if (!paramFound) {
			ServletUtils.addError(PARAM_BLACKLIST_USER,
					"Nevybral jste žádného uživatele.", env, null);
			return false;
		}
		return true;
	}
	...
	/**
	 * Subscribes user to monthly summary from parameters. Changes are not
	 * synchronized with persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean setMonthlySummary(Map params, User user) {
		...
		Element element = DocumentHelper.makeElement(user.getData(),
				"/data/communication/email/newsletter");
		String value = ("yes".equals(subscription)) ? "yes" : "no";
		element.setText(value);
		return true;
	}
	...
	/**
	 * Subscribes user to email forum from parameters. Changes are not
	 * synchronized with persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean setForumByEmail(Map params, User user) {
		...
		Element element = DocumentHelper.makeElement(user.getData(),
				"/data/communication/email/forum");
		String value = ("yes".equals(subscription)) ? "yes" : "no";
		element.setText(value);
		if ("yes".equals(value))
			SubscribedUsers.getInstance()
					.addUser(user.getId(), user.getEmail());
		else
	...
	/**
	 * Sets page flow for admin, when he move discussion from parameters.
	...
	/**
	 * Sets user roles from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean setUserRoles(Map params, User user) {
		Element nodeRoles = (Element) user.getData().selectSingleNode(
				"/data/roles");
		if (nodeRoles != null)
			nodeRoles.detach();
	...
	/**
	 * Sets selected feeds from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @return false, if there is a major error.
	 */
	private boolean setFeeds(Map params, User user) {
		Element element = (Element) user.getData().selectSingleNode(
				"/data/settings/feeds");
		if (element != null)
			element.detach();
		...
		DocumentHelper.makeElement(user.getData(), "/data/settings/feeds")
				.setText(sb.toString());
		return true;
	}

	/**
	 * Overrides number of links per feed. Changes are not synchronized with
	 * persistence.
	 * 
	 * @return false, if there is a major error.
	 */
	private boolean setFeedSize(Map params, User user, Map env) {
		...
		boolean result = setLimitedSize(params, PARAM_INDEX_FEED_SIZE, user
				.getData(), "/data/settings/index_links", 1, max, env);
		max = (Integer) maxSizes.get(VariableFetcher.KEY_TEMPLATE_LINKS);
		result &= setLimitedSize(params, PARAM_TEMPLATE_FEED_SIZE, user
				.getData(), "/data/settings/template_links", 1, max, env);
		return result;
	}

	/**
	 * Uploads photo from parameters. Changes are not synchronized with
	 * persistence.
	 * 
	 * @param params
	 *            map holding request's parameters
	...
	private boolean setPhoto(Map params, User user, Map env) {
		...
		return ImageTool.storeImage(User.USER_PHOTO, image, user,
				ImageTool.USER_PHOTO_RES, env, PARAM_PHOTO);
	}
	...
	private boolean setGPG(Map params, User user, Map env) throws Exception {
		...
		if (Misc.empty(key)) {
			...
			if (node != null) {
				String localPath = AbcConfig.calculateDeployedPath(node
						.getText());
		...
		BufferedWriter out = new BufferedWriter(new FileWriter(AbcConfig
				.calculateDeployedPath(fileName)));
		out.write(key);
		out.close();

		Element gpg = DocumentHelper.makeElement(user.getData(),
				"/data/profile/gpg");
	...
	private boolean setForumMode(Map params, User user) {
		...
		if ("single".equals(mode)) {
			Element emode = DocumentHelper.makeElement(user.getData(),
					"/data/profile/forum_mode");
			emode.setText(mode);
		} else {
			Node node = user.getData().selectSingleNode(
					"/data/profile/forum_mode");
			if (node != null)
				node.detach();
		}
	...
	private boolean setAvatar(Map params, User user, Map env) {
		...
		return ImageTool.storeImage(User.USER_AVATAR, image, user,
				ImageTool.USER_AVATAR_RES, env, PARAM_AVATAR);
	}
	 ...
	 * @param xpath
	 *            xpath to be updated within document
	 * @param min
	 *            minimum allowed valued
	 * @param max
	 *            maximum allowed valued
	 * @param env
	 *            possible error message wil be stored there
	 * @return false in case of major error
	 */
	private boolean setLimitedSize(Map params, String paramName,
			Document document, String xpath, int min, int max, Map env) {
		...
		if (limit == null || limit.length() == 0) {
			...
			if (tmp < min || tmp > max) {
				ServletUtils.addError(paramName, "Zadejte číslo v rozsahu "
						+ min + " - " + max + "!", env, null);
				return false;
			}
	...
	/**
	 * @param html
	 *            non-empty string that may contain HTML
	 * @return number of text characters (length of string stripped of HTML
	 *         tags)
	 */
