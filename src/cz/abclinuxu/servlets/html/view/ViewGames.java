...
public class ViewGames implements AbcAction {
    ...
    public static String playTriviaGame(HttpServletRequest request, Relation relation, Map env) throws Exception {
        ...
        if (game.getType() != Item.TRIVIA)
            throw new InvalidInputException("Tato relace nepatří kvízu!");
        ...
        if (position == -1) {
            ...
            if (Misc.empty(lastChoice))
                ServletUtils.addError(Constants.ERROR_GENERIC, "Prosím, vyberte jednu z nabízených možností.", env, null);
            else
                position++;
        }
