...
public class SelectUser implements AbcAction {
    ...
    protected String actionShowFoundUsers(HttpServletRequest request, Map env, Map params) throws Exception {
        ...
        if (searched.preciseEquals(new User())) {
            ServletUtils.addError(Constants.ERROR_GENERIC,"Vyplňte alespoň jedno políčko!",env,null);
            return actionShowForm(request, env, params);
        }
    ...
    ///////////////////////////////////////////////////////////////////////////
     ...
     private boolean setCity(Map params, User user, Map env) {
         ...
         if ( city.length()>0 && city.length()<3 ) {
             ServletUtils.addError(PARAM_CITY, "Bydliště musí obsahovat nejméně tři písmena!", env, null);
             return false;
         }
    ...
    private boolean setName(Map params, User user, Map env) {
        ...
        if ( name.length()>0 && name.length()<3 ) {
            ServletUtils.addError(PARAM_NAME, "Jméno musí obsahovat nejméně tři písmena!", env, null);
            return false;
        }
        user.setName("%"+name+"%");
        return true;
    ...
    private boolean setLogin(Map params, User user, Map env) {
        ...
        if ( login.length()>0 && login.length()<3 ) {
            ServletUtils.addError(PARAM_LOGIN, "Přihlašovací jméno musí obsahovat nejméně tři písmena!", env, null);
            return false;
        }
        user.setLogin("%"+login+"%");
        return true;
    ...
    private boolean setEmail(Map params, User user, Map env) {
        ...
        if ( email.length()>0 && (email.length()<3 || email.indexOf('@')==-1) ) {
            ServletUtils.addError(PARAM_EMAIL, "Email musí obsahovat nejméně tři písmena a zavináč!", env, null);
            return false;
        }
        user.setEmail("%"+email+"%");
        return true;
    ...
    private boolean setId(Map params, User user, Map env) {
        ...
        try {
            ...
            ServletUtils.addError(PARAM_USER, "Zadejte číslo!", env, null);
            return false;
        }
    }
