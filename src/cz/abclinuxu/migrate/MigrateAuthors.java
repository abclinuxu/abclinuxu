 ...
 * only storage format differs, so items are just updated.
 * 
...
public class MigrateAuthors {
	 ...
	 * 
	 * @param parent
	 *            Parent to get new child
	 * @param name
	 *            Child's name
	 * @param value
	 *            Text value of child
	 * @throws InvalidDataException
	 *             When there is data conflict between datum included and data
	 *             added.
	 */
	private void safeAppendElement(Element root, String xpath, String value)
			throws InvalidDataException {
		...
		// there is node, what to do if its value differs from one to be set
			...
			throw new InvalidDataException("Author already contains node of "
					+ root.getPath() + "/" + xpath + ", with value: "
					+ tmp.getText() + ", which differs from: " + value);
		}
	}
	 ...
	 *            Root element
	 * @param xpath
	 *            XPath path to determine removed element
	 */
	...
	private int determineActivity(int authorId) {
		...
		cal.set(Calendar.MONTH, cal.get(Calendar.MONTH)
				- MONTHS_TO_SET_INACTIVE);
		java.sql.Date after = new java.sql.Date(cal.getTimeInMillis());

		final Qualifier[] qualifiers = { new CompareCondition(Field.CREATED,
				Operation.GREATER, after) };

		// no qualifiers for count functions are present
		// just fetch relations list and chect size
		List<Relation> relations = sqlTool.findArticleRelationsByAuthor(
				authorId, qualifiers);
